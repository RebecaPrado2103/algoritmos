...
function salvar() {
    ...
    if (id && titulo) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                filme = new Filme(id, titulo, diretor, dataDeLancamento, avaliacao);
                listaFilme.push(filme);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                filmeAlterado = new Filme(id, titulo, diretor, dataDeLancamento, avaliacao);
...
function mostrarDadosFilme(filme) {
    ...
    document.getElementById("inputdataDeLancamento").value = filme.dataDeLancamento;
...
function filtrarPorAvaliacao(){
    ...
    document.getElementById("outputSaida").innerHTML = preparaListagem(acimaDeOito);
}
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaFilme.length; i++) {
        const linha = listaFilme[i]; //variavel linha contem as informações de cada pet
        textoCSV += linha.id + ";" +
            linha.titulo + ";" +
            linha.diretor + ";" +
            linha.dataDeLancamento + ";" +
