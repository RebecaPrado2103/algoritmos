...
function procure() {
    const numeroMac = document.getElementById("inputNumeroMac").value;
    if (isNaN(numeroMac) || !Number.isInteger(Number(numeroMac))) {
        mostrarAviso("Precisa ser um número inteiro");
        document.getElementById("inputNumeroMac").focus();
        return;
    }

    if (numeroMac) { // se digitou um Id
        celular = procurePorChavePrimaria(numeroMac);
...
function salvar() {
    ...
    let numeroMac;
    if (celular == null) {
        numeroMac = parseInt(document.getElementById("inputNumeroMac").value);
    } else {
        numeroMac = celular.numeroMac;
    }
    ...
    if (numeroMac && modelo) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                celular = new Celular(numeroMac, modelo, fabricante, preco);
                listaCelular.push(celular);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                celularAlterado = new Celular(numeroMac, modelo, fabricante, preco);
...
function mostrarDadosCelular(celular) {
    document.getElementById("inputNumeroMac").value = celular.numeroMac;
...
function bloquearAtributos(soLeitura) {
    ...
    document.getElementById("inputNumeroMac").readOnly = !soLeitura;
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    let nomeDoArquivoDestino = "./Celular.csv";  //define o nome do arquivo csv
    let textoCSV = "";
    for (let i = 0; i < listaCelular.length; i++) {
        const linha = listaCelular[i]; //variavel linha contem as informações de cada pet
        textoCSV += linha.numeroMac + ";" +
            linha.modelo + ";" +
            linha.fabricante + ";" +
            linha.preco + "\n";
    }
...
function converterDeCSVparaListaObjeto(arquivo) {
    ...
    leitor.onload = function (e) {
        ...
        listaCelular = []; // Limpa a lista atual (se necessário)
