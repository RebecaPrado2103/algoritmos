...
function procurePorChavePrimaria(chave) {
    for (let i = 0; i < listaPapelaria.length; i++) {
        const produto = listaPapelaria[i];
        if (produto.id == chave) {
...
function salvar() {
    ...
    if (id && nome) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                produto = new Papelaria(id, nome, categoria, quantidadeEstoque, precoUnitario);
                listaPapelaria.push(produto);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                produtoAlterado = new Papelaria(id, nome, categoria, quantidadeEstoque, precoUnitario);
                listaPapelaria[produto.posicaoNaLista] = produtoAlterado;
...
function mostrarDadosLivraria(produto) {
    document.getElementById("inputId").value = produto.id;
    document.getElementById("inputNome").value = produto.nome;
    document.getElementById("Categoria").value = produto.categoria;
    document.getElementById("inputquantidadeEstoque").value = produto.quantidadeEstoque;
    document.getElementById("inputprecoUnitario").value = produto.precoUnitario;
...
function bloquearAtributos(soLeitura) {
    ...
    document.getElementById("Categoria").readOnly = soLeitura;
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaPapelaria.length; i++) {
        const linha = listaPapelaria[i]; //variavel linha contem as informações de cada pet
        textoCSV += linha.id + ";" +
            linha.nome + ";"+
            linha.categoria + ";" +
            linha.quantidadeEstoque + ";" +
            ...
            if (linha) { //verifica se a linha não está vazia
                ...
                if (dados.length === 2) { //verifica os seis campos
                    ...
                    listaPapelaria.push({
                        ...
                        nome: dados[1],
                        categoria: dados[2],
                        quantidadeEstoque: dados[1],
                        precoUnitario: dados[1]
                    });
                }
            }
        }
