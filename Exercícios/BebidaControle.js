let listaBebida = []; //conjunto de dados
let oQueEstaFazendo = ''; //variável global de controle
let bebida = null; //variavel global 
bloquearAtributos(true);
//backend (não interage com o html)
function procurePorChavePrimaria(chave) {
    for (let i = 0; i < listaBebida.length; i++) {
        const bebida = listaBebida[i];
        if (bebida.id == chave) {
            bebida.posicaoNaLista = i;
            return listaBebida[i];
        }
    }
...
function procure() {
    ...
    if (id) { // se digitou um Id
        bebida = procurePorChavePrimaria(id);
        if (bebida) { //achou na lista
            mostrarDadosBebida(bebida);
...
function salvar() {
    ...
    const precoEtiqueta = document.getElementById("inputPrecoEtiqueta").value;
    const ehAlcolico = document.getElementById("inputehAlcolico").checked;
    //verificar se o que foi digitado pelo USUÁRIO está correto
    if (id && nome) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                bebida = new Bebida(id, nome, fabricante, precoEtiqueta, ehAlcolico);
                listaBebida.push(bebida);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                bebidaAlterado = new Bebida(id, nome, fabricante, precoEtiqueta, ehAlcolico);
                listaBebida[bebida.posicaoNaLista] = bebidaAlterado;
                mostrarAviso("Alterado");
                break;
            case 'excluindo':
                let novaLista = [];
                for (let i = 0; i < listaBebida.length; i++) {
                    if (bebida.posicaoNaLista != i) {
                        novaLista.push(listaBebida[i]);
                    }
                }
                listaBebida = novaLista;
...
function listar() {
    document.getElementById("outputSaida").innerHTML = preparaListagem(listaBebida);
}
...
function mostrarDadosBebida(bebida) {
    document.getElementById("inputId").value = bebida.id;
    document.getElementById("inputNome").value = bebida.nome;
    document.getElementById("inputFabricante").value = bebida.fabricante;
    document.getElementById("inputPrecoEtiqueta").value = bebida.precoEtiqueta;
    document.getElementById("inputehAlcolico").checked = bebida.nome;
...
function limparAtributos() {
    ...
    document.getElementById("inputPrecoEtiqueta").value = "";
    document.getElementById("inputehAlcolico").checked = false;
...
function bloquearAtributos(soLeitura) {
    ...
    document.getElementById("inputPrecoEtiqueta").readOnly = soLeitura;
    document.getElementById("inputehAlcolico").readOnly = soLeitura;
}
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    let nomeDoArquivoDestino = "./Bebida.csv";  //define o nome do arquivo csv
    let textoCSV = "";
    for (let i = 0; i < listaBebida.length; i++) {
        const linha = listaBebida[i]; //variavel linha contem as informações de cada pet
        textoCSV += linha.id + ";" +
            linha.nome + ";" +
            linha.fabricante + ";" +
            linha.precoEtiqueta + ";" +
...
function converterDeCSVparaListaObjeto(arquivo) {
    ...
    leitor.onload = function (e) {
        ...
        listaBebida = []; // Limpa a lista atual (se necessário)
