...
function AnimalMaisPesado(){
    let MaisPesado = listaAnimal[0];
    for(let i=0; i<listaAnimal.length; i++){
        let a = listaAnimal[i]
        if(a.peso > MaisPesado.peso){
            MaisPesado = a
        }
    }
    document.getElementById("outputSaida").innerHTML = preparaListagem([MaisPesado]);
}
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaAnimal.length; i++) {
        const linha = listaAnimal[i]; //variavel linha contem as informações de cada pet
        textoCSV += linha.id + ";" +
        linha.especie + ";" +
        linha.nome + ";" +
        linha.idade + ";" +    
