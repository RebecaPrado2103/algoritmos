...
function procure() {
    ...
    if (codigo) { // se digitou um Id
        livro = procurePorChavePrimaria(codigo);
        if (codigo) { //achou na lista
...
function salvar() {
    ...
    const anoPublicacao = parseInt(document.getElementById("inputanoPublicacao").value);
    const genero = document.getElementById("inputGenero").value;
    //verificar se o que foi digitado pelo USUÁRIO está correto
    if (codigo && titulo) {// se tudo certo 
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaLivro.length; i++) {
        const linha = listaLivro[i]; //variavel linha contem as informações de cada pet
