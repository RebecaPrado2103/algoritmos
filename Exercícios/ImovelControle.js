...
function procure() {
    ...
    if(numMatricula<=0){
        alert("Número da matricula não pode ser um número negativo ou igual a zero")
    }

    if (numMatricula) { // se digitou um Id
...
function salvar() {
    ...
    if (numMatricula && endereco) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                imovel = new Imovel(numMatricula, endereco, bairro, tipo, area, comodos, alugado, data);
                listaImovel.push(imovel);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                imovelAlterado = new Imovel(numMatricula, endereco, bairro, tipo, area, comodos, alugado, data);
...
function mostrarDadosImovel(imovel) {
    ...
    document.getElementById("inputAlugado").checked = imovel.alugado;
    document.getElementById("inputData").value = imovel.data;
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaImovel.length; i++) {
        const linha = listaImovel[i]; //variavel linha contem as informações de cada pet
